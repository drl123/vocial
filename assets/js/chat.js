...
const handlePresenceDiff = diff => {
  ...
  // Next, based on the diff, get the new presence of the presences variable
  presences = Presence.syncDiff(presences, diff);
  // Sync up the user list to the new state
  syncUserList(presences);
  // For all new statuses, and status messages to the chat log.
...
const connect = (socket, username) => {
  ...
  // Create a channel to handle the joining/sending/receiving
  ...
  channel
    ...
    .receive("error", res => console.log("Failed to join the channel:", res));
