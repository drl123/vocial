defmodule Vocial.Votes.VotesTest do
  use Vocial.DataCase
  ...
  describe "polls" do
    ...
    test "creat_poll_with_options/2 does not create the poll or options with invalid data", %{user: user} do
      title = "Bad Poll"
  ...
  describe "options" do
    test "create_option/1 creates and option on a poll", %{user: user} do
  ...
  describe "messages" do
    ...
    setup(%{user: user}) do
      ...
      Enum.each(poll_messages,  fn m -> Votes.create_message(%{ message: m, author: "Someone", poll_id: poll.id }) end)
    ...
    test "list_lobby_messages/0 only includes lobby messages" do
    ...
    test "list_poll_messages/1 only includes poll messages", %{poll: poll} do
      ...
      assert Enum.count(Votes.list_poll_messages(poll.id), &(&1.poll_id == poll.id))
    end
  end
end
